...
export default function BulkPrintPage() {
  ...
  function handlePrint() {
    ...
    const labelHtml = result.created.map(label => {
      ...
      return `
        <div class="label">
          ...
          ${product.designNumber ? `<div class="sub">Design ${product.designNumber}</div>` : `<div class="sub">${product.sku}</div>`}
          ${dataUrl ? `<img src="${dataUrl}" class="qr" />` : `<div class="qr-placeholder">QR</div>`}
          <div class="qty">${label.quantity} pcs</div>
    ...
    if (!win) return;
    win.document.write(`<!DOCTYPE html><html><head><title>Bulk Labels — ${product.name}</title>
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:-apple-system,sans-serif; background:#fff; padding:10mm; }
  ...
  .name { font-size:9px; font-weight:700; color:#1e3a5f; line-height:1.2; margin-bottom:2px; }
  .sub  { font-size:8px; color:#64748b; margin-bottom:4px; }
  .qr   { width:36mm; height:36mm; display:block; margin:0 auto 4px; }
  .qr-placeholder { width:36mm; height:36mm; background:#f1f5f9; margin:0 auto 4px; display:flex; align-items:center; justify-content:center; font-size:8px; color:#94a3b8; }
  .qty  { font-size:13px; font-weight:900; color:#1d4ed8; margin-bottom:1px; }
